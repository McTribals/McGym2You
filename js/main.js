...
const componenteLaboral = ` <b>COMPONENTES DEL PLAN </b> <br/>
                           ...
                           <img src="images/Logo/chevron-sign-to-right.png" style="width: 15px;"/>	Actividad de integración laboral.`;
...
/*TENDENCIAS*/
/*Title Tendencias*/
const suspension = 'SUSPENSION / TRX';
...
const hiperCaloricosText = `Los hipercalóricos son suplementos en polvo, generalmente destinados a la personas con dificultades para ganar masa muscular, que poseen un metabolismo alto y con un somatotipo delgado, son suplementos altamente calóricos y con cantidades medias de grasas, teniendo una importancia relevante para aquellos que deseen aumentar su masa muscular, están compuestos de mayores cantidades de carbohidratos, medianas cantidades de proteínas y lípidos contienen vitaminas, minerales y nutrientes indispensables para que los músculos crezcan en su nivel máximo`;
const hipoCaloricosText = `Son proteínas de más bajo aporte calórico por lo que se les considera como proteínas limpias ya que su contenido en carbohidratos y grasa tiende a ser muy bajo, este tipo son los más indicados para las personas que desean mantener la masa muscular en niveles de grasa muy bajos, siendo así una de las proteínas de mayor calidad para el crecimiento y fuerza muscular magra, es ideal para personas con tendencia a la endomorfia “ Subir de peso con facilidad ” o atletas que realizan dietas bajas en calorías en busca de un porcentaje de definición muscular.`;
const ceroCarbohidratosText = `Este tipo de suplementos se caracteriza por carecer de carbohidratos y azucares además de contener un alto perfil proteico, razón por la cual su contenido calórico es mínimo y este procede única y exclusivamente de las proteínas presentes en este, es ideal para personas diabéticas debido a su ausencia de azucares y carbohidratos, además contribuye con el desarrollo de la masa muscular limpia y es ideal en procesos de perdida de peso y definición muscular.`;
const quemadoresText = `Este tipo de suplementos están diseñados para incrementar la perdida de peso y específicamente la reducción de tejido graso, cabe notar que existen dos tipos de quemadores de grasa. `;
const energiaText = `Esta línea de suplementos permite proporcionar amplia energía que contribuyen con el buen desempeño físico, también contamos con una línea de suplementos recuperadores que contribuyen a la reparación de los tejidos musculares reducen la fatiga corporal.`;
const naturistaText = `Nuestra tienda en línea cuenta con una amplia variedad de productos dietéticos y naturistas de alta calidad, diseñados para satisfacer las necesidades de nuestros clientes a la vez que aportan bienestar y salud, aquí podrás encontrar vitaminas, minerales, oligoelementos y diuréticos, entre muchos otros productos de líneas naturales, libres de químicos y de procesos de tipo farmacéutico que garantizan resultados sin efectos secundarios para la salud.`;
const gelesText = `La línea reductora de SkinSlim fue científicamente desarrollada durante un proceso de 5 años de investigación y perfeccionamiento que garantizan óptimos resultados, sin generar efectos secundarios, es ideal para reafirmar el tejido muscular y mejorar los procesos de definición corporal.`;
const accesoriosFitnessText = `Contamos con una amplia gama de elementos para la realización de actividad física y accesorios deportivos, productos de alta calidad que garantizan su durabilidad, resistencia y ergonomía para la buena practica de actividades físico deportivas en la comodidad de su hogar o al aire libre.`;

/*Beneficios suplementos y accesorios*/

const hiperCaloricosBeneficio = `<img src="images/Logo/maps-and-flags.png"/> Proporciona ganancia de peso para personas con metabolismo acelerado. <br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Ayudan a la ganancia de volumen de masa muscular y peso.<br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Suplen las necesidades de energía.<br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Son una fuente importante de vitaminas y minerales.<br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Acelera la recuperación Post-Entrenamiento.`;
const hipoCaloricosBeneficio = ` <img src="images/Logo/maps-and-flags.png"/> Ideales para programas de perdida de peso.<br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Aportan altas dosis de proteína.<br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Muy bajos en grasa, azucares y carbohidratos.<br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Son una fuente importante de vitaminas y minerales.<br/>
                                 <img src="images/Logo/maps-and-flags.png"/> Contribuyen al crecimiento de la masa muscular limpia.`;
...
function showModalPlans(element){
   console.log('Entro en metodo ' + element.id);
      ...
      }else if(idElement == 'pausasLaborales'){ displayModal(idElement, pausasLaborales, pausasLaboralesImage, pausasLaboralesText, beneficioLaboral );
      }else if(idElement == 'nutricionYDietetica'){displayModal(idElement, nutricionYDietetica, nutricionYDieteticaImage, nutricionYDieteticaText);
      ...
      }else if(idElement == 'preNatal'){ displayModal(idElement, preNatal , preNatalImage, preNatalText);
      }else if(idElement == 'hiperCaloricos'){ displayModal(idElement, hiperCaloricos, hiperCaloricosImages, hiperCaloricosText);
      }else if(idElement == 'hipoCaloricos'){displayModal(idElement, hipoCaloricos, hipoCaloricosImages, hipoCaloricosText);
...
function displayModal(id, plan, image, text, compo, bene){
   ...
   if(bene == 'undefined'){
      $('#mainModal .modal-body #content-modal').html(`
                                                      <p>${text}<hr>${compo}</p>`);
   }else{
      $('#mainModal .modal-body #content-modal').html(`
                                                      <p>${text}<hr>${compo}</p>${bene}`);
   }
